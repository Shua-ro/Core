...
const totalValueGroceryMain = document.querySelector(".groceries-value");
const totalValueGrocery = document.querySelector(".grocery-expense-value");
/* MAIN FUNCTION */
async function loadGroceryData(page) {
  const entries = await window.electronAPI.getGrocery();
  const total = await groceryTotal();
  totalValueGrocery.innerText = "₱" + total;
  totalValueGroceryMain.innerText = "₱" + total;
